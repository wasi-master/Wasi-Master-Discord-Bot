...
await ctx.send(f"There were {
	...
	with open("prefixes.json", "r") as f:
	prefixes = json.load(f)
	await ctx.send(f"The prefixes file has {
		len(prefixes)} servers")
	...
	response = requests.get(f"http://www.thecolorapi.com/id?hex= {
		hex
	}")
	data = json.loads(response.text)
	color_name = data.get("name").get("value")
	link = f"http://singlecolorimage.com/get/ {
		hex
	}/1x1"
	...
	embed.set_footer(text = f"Made for {
		ctx.author
	}")
	...
	await ctx.send(f"prefix set to ` {
		prefix
	}`")
	with open("prefixes.json", "w") as f:
	json.dump(prefixes, f,  indent = 4)
	...
	embed.set_author(name = f'Role Changed for {
		member
	}')
	embed.set_footer(text = f"Done by {
		ctx.author
	}")
	embed.add_field(name = "Removed Role", value = f'@ {
		role
	}')
	await ctx.send(embed = embed)
	else :
	...
	embed.add_field(name = "Added Role", value = f'@ {
		role
	}')
	await ctx.send(embed = embed)
	...
	embed.add_field(name = "How Gay?", value = f" {
		member.name
	} is {
		random.randint(0, 100)}% gay")
	await ctx.send(embed = embed)
	...
	for i in mesglist:
	num += 1
	embed.add_field(name = f"Choice {
		num
	}", value = f" {
		i
	}")
	embed.add_field(name = "‌", value = "‌")
	embed.add_field(name = f"**Chosen**", value = f" {
		random.choice(mesglist)}")
	await ctx.send(embed = embed)
	...
	embed.add_field(name = "Ping", value = f'Pong! I got your message after {
	...
	async with ctx.typing():
	embed = discord.Embed(timestamp = ctx.message.created_at)
	embed.set_author(name = f"Synonyms for {
		args
	}")
	response = requests.get(f"https://dictionary.yandex.net/api/v1/dicservice.json/lookup?key= {
		api_key
	}&lang=en-en&text= {
		args.lower()}")
	data = response.json()
	...
	for i in synonyms:
	num += 1
	embed.add_field(name = f"Synonym {
		num
	}", value = i.get("text"), inline = True)
	except:
	...
	headers = {
		"x-rapidapi-host": "mashape-community-urban-dictionary.p.rapidapi.com","x-rapidapi-key": "1cae29cc50msh4a78ebc8d0ba862p17824ejsn020a7c093c4d"
	}
	num = 0
	embed = discord.Embed(timestamp = ctx.message.created_at)
	...
	embed.add_field(name = f"Definition {
		num
	}", value = i.get("definition").replace("[", "**").replace("]", "**"))
	else :
	...
	if not lang == "help":
	try:
	response = requests.get(f"https://translate.yandex.net/api/v1.5/tr.json/translate?key=trnsl.1.1.20200414T062446Z.1e5abaa65939d784.390d015d69abbe56445b9ba840e7b556c709efd2&text= {
		args
	}&lang= {
		lang
	}")
	...
	embed.add_field(name = f"Translation of {
		args
	}", value = translation)
	except:
	...
	await ctx.send(f"https://discord.com/channels/ {
	...
	overwrites = {
		ctx.guild.default_role: discord.PermissionOverwrite(read_message_history = False),
	...
	embed.add_field(name = f" {
		prefix
	}help", value = 'shows this help message')
	embed.add_field(name = f" {
		prefix
	}ping", value = 'shows the latency of the bot')
	embed.add_field(name = f" {
		prefix
	}8ball `<question>`", value = 'genarates a answer to a question')
	embed.add_field(name = f" {
		prefix
	}clear `<amount>`", value = 'clears a amount of messages')
	embed.add_field(name = f" {
		prefix
	}kick `<@mention>`", value = 'kicks a member')
	embed.add_field(name = f" {
		prefix
	}ban `<@mention>`", value = 'bans a member')
	embed.add_field(name = f" {
		prefix
	}userinfo `<@mention>`", value = 'shows info about a user')
	embed.add_field(name = f" {
		prefix
	}say `<string>`", value = 'the bot says anything you type after, say')
	embed.add_field(name = f" {
		prefix
	}avatar `<@mention>`", value = 'shows the avatar of the user that you mention after ,avatar')
	embed.add_field(name = f" {
		prefix
	}choose `<items separated by commas>`", value = 'chooses an item from the items you say and separate by commas after ,choose')
	embed.add_field(name = f" {
		prefix
	}invite", value = 'sends the bot invite lnk')
	embed.add_field(name = f" {
		prefix
	}howgay `<@mention>`", value = 'shows how gay a user is')
	embed.add_field(name = f" {
		prefix
	}role `<@mention>`", value = 'Changes role for a user')
	embed.add_field(name = f" {
		prefix
	}prefix `<prefix>`", value = 'Used to set a custom prefix')
	embed.add_field(name = f" {
		prefix
	}google `<query>`", value = 'Used to search google without having to open google and search manually')
	embed.add_field(name = f" {
		prefix
	}synonyms `<word>`", value = 'Returns the synonyms of a word')
	embed.add_field(name = f" {
		prefix
	}urbandictionary `<word>`", value = 'Retyrns the definitions of a word found in urban dictionary')
	embed.add_field(name = f" {
		prefix
	}define `<word>`", value = 'Returns the definitions of a word found in merriam webster')
	embed.add_field(name = f" {
		prefix
	}translate `<language>` `<text>`", value = 'Returns the translation of a text found in Yandex')
	embed.add_field(name = f" {
		prefix
	}quiz", value = 'Used to get a quiz (not fully made)')
	embed.add_field(name = f" {
		prefix
	}link", value = 'Used to get a link to the message')
	embed.add_field(name = f" {
		prefix
	}dm `<text>`", value = 'Used to send the person writing this command a dm which can be used to remember something')
	embed.add_field(name = f" {
		prefix
	}getusers `<@role>`", value = "used to get users that have a specefic oile")
	await ctx.send(embed = embed)
	...
	await ctx.send(f"I\'m in {
		servers
	} servers and there are {
		members
	} members total d there are {
		members
	} members total and {
		average
	}  on average in each server")
	...
	message = await ctx.send(f"deleted ` {
		len(deleted)}`' messages")
	time.sleep(4)
	...
	await ctx.send(f'Nickname was changed for {
		member.mention
	} ')
