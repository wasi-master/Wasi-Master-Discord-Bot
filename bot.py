...
async def spotify(ctx, member: discord.Member=None):
	...
	for activity in activity.activities:
		if isinstance(activity, discord.Spotify):
			...
			embed.add_field(name="Time", value=f"{convert_sec_to_min((datetime.now() - activity.start).total_seconds())}  {get_p(((datetime.now() - activity.start).total_seconds())/((datetime.now() - activity.end)).total_seconds()/100)}  {str(activity.duration)[2:-7]}")
