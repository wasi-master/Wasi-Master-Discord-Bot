...
async def firstmessage(ctx, channel: discord.TextChannel = None):
    ...
    embed.add_field(
        name="Message sent at", value=fmo.created_at.strftime("%a, %d %B %Y, %H:%M:%S")
    )
    if not fmo.edited_at is None:
        embed.add_field(
            name="Edited", value=fmo.edited_at.strftime("%a, %d %B %Y, %H:%M:%S")
        )
...
async def helpcommand(ctx, command: str = None):
    ...
    if command is None:
        ...
        embed = discord.Embed(
            colour=ctx.guild.me.color or None,
...
async def userinfo(ctx, *, member: discord.Member = None):
    ...
    embed.add_field(
        name="Created at", value=member.created_at.strftime("%a, %d %B %Y, %H:%M:%S")
    )

    embed.add_field(
        name="Joined at:", value=member.joined_at.strftime("%a, %d %B %Y, %H:%M:%S")
    )
