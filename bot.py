...
async def help(ctx, command: str=None):
	...
	if command is None:
		...
		if command.strip().lower() in all_commands_name_list:
			...
			embed = discord.Embed(title=f'Command "{str(command)}" was not found, try using the command name instead of it\'s alias', description=f"Did you mean {difflib.get_close_matches(command.strip().lower(), all_commands_name_list)[0], n=1, cutoff=0.8}")
			await ctx.send(embed=embed)
